...
for($i=1; $i<=$total_page; $i++){
    echo "<span class='pagination'><a href='date_month.php?year=$year&id=$month_year&page=$i'>$i</a></span>";
}
?>
<!--pagination-->			
        </div>
                    ...
                    if($year == $value->year){
                        ...
                        if($resource){
                            while($month = $resource->fetch_object()){
                                ?>
                                    <li><a href="date_month.php?id=<?php echo $month->month_year."&year=$year";?>"><?php echo formatMonth($month->month_year)."($month->num_post)";?></a></li>
                                <?php
                            }
                        }
                    }
